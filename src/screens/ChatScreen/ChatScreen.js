...
class ChatScreenComponent extends Component {
  ...
  componentDidMount = () => {
    ...
    // console.log('primaryActorDetails',primaryActorDetails,typeof primaryActorDetails,conversationId);
    // Clear all notification related the conversation
    if (primaryActorDetails) {
      const actorDetails = typeof primaryActorDetails === 'string' ? JSON.parse(primaryActorDetails) : primaryActorDetails
      this.props.markNotificationAsRead({
        primaryActorId: actorDetails.primary_actor_id,
        primaryActorType: actorDetails.primary_actor_type,
      });
    }
  ...
  onBackPress = () => {
    const { navigation } = this.props;
    this.props.route.params?.primaryActorDetails ? navigation.replace('Tab') : navigation.goBack()
  };
  loadMoreMessages = () => {
  ...
  renderMessage = item => (
    <ChatMessage message={item.item} key={item.index} index={item.index} showAttachment={this.showAttachment}/>
  );
  ...
  render() {
    ...
    // console.log(JSON.stringify(groupedConversationList.data[groupedConversationList.data.length-1]))
    return (
