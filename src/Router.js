...
import { Linking, SafeAreaView, KeyboardAvoidingView, Platform, Alert } from 'react-native';
...
const Router = ({ eva: { style } }) => {
  ...
  // }, [linkedURL, resetURL]);
  const linking = {
    prefixes: ["karzounchat://"],
    config: {
      screens: {
        ChatScreen: {
          path:'chat/:conversationId/:primaryActorDetails?',
          parse:{
...
export default withStyles(Router, styles);
